...
export class TopYearCrimes extends Transform {
    ...
    _flush(callback) {
        ...
        const sorted = new Map([...this.crimesPerYear.entries()].sort())
        let result = 'same';
        let keys = Array.from(sorted.keys());

        if (sorted.get(keys[0]) > sorted.get(keys[keys.length])) result = 'up'
        else if (sorted.get(keys[0]) < sorted.get(keys[keys.length])) result = 'down'
